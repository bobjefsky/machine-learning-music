...
def main():
    n_epochs = 16
    print("using " + str(n_epochs) + " epochs")
    name_of_song = 'gravityFalls.mid'

    #data_fn = 'mid_songs/' + 'original-methany.mid'  # 'And Then I Knew' by Pat Metheny
    data_fn = 'mid_songs/' + name_of_song  # 'Mid Version of Chihiro by Yoste from Spotify
    #data_fn = 'mid_songs/' + 'Gravity_Falls_Theme.mid'

    out_fn = 'output_songs/' + name_of_song[:-4] + str(n_epochs)
    if (n_epochs == 1):
        out_fn += '_epoch.midi'
    else:
        out_fn += '_epochs.midi'
