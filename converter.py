"""
souce:  https://github.com/trevorwelch/chroma-to-midi
...
def main():
    print("converting .mp3 to .mid")

    # for path of files
    path_to_mp3s = "\mp3_songs"
    path_to_mids = "\mid_songs"
    current_directory = os.getcwd()
    full_path_to_mp3s = current_directory + path_to_mp3s
    full_path_to_mids = current_directory + path_to_mids

    # for file conversion
    dcp = madmom.audio.chroma.DeepChromaProcessor()
    tempo_var = 130

    for file in os.listdir(full_path_to_mp3s):
        if file.endswith(".mp3"):
            path_to_single_mp3 = os.path.join(full_path_to_mp3s, file)
            path_to_new_mid = os.path.join(full_path_to_mids, file[:-4] + '.mid')
            chroma = dcp(os.path.join(path_to_single_mp3))
            chroma_out = threshhold_chroma(chroma)
            chroma_to_midi(chroma_out, path_to_new_mid, tempo_var)

            #once converted, delete the mp3 file
            remove.remove(path_to_single_mp3)
...
def chroma_to_midi(chromagram, path_to_midi, tempo_var):
